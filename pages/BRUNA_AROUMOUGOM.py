...
def chiffres_affaires():
    ...
    st.title("2048 - BRUNA AROUMOUGOM : Nettoyage du Chiffres d'Affaires")
...
def reglements():
    # Afficher le titre personnalisé
    st.title("2048 - BRUNA AROUMOUGOM : Nettoyage des Règlements")
    ...
    if uploaded_file is not None:
        ...
        column_names = ["Date", "N°règlement", "Encaissé par", "Pièce", "Nom", "Prénom", "Date de naissance", "Genre", "Origine", "Mobile", "Email", "Statut", "CB", "CB en ligne", "Espèces", "Chèques", "Autres", "Total hors chèques cadeaux", "Chèques cadeaux", "Total réglé", "Impayés"]
        df.columns = column_names
        df['N°compte'] = df.apply(lambda row: "C" + str(row['Nom'])[:3] + str(row['Prénom'])[:3], axis=1)
        df['Libellé'] = df.apply(lambda row: f"{row['Nom']} {row['Prénom']}", axis=1)
        df_short = df[["Date", "Pièce", "N°compte", "Libellé", "CB", "CB en ligne", "Espèces", "Chèques", "Autres", "Chèques cadeaux", "Total réglé"]]
        df_short = df_short.dropna()
        ...
        df_TTC.drop(["CB", "CB en ligne", "Espèces", "Chèques", "Autres", "Chèques cadeaux", "Total réglé"], axis=1, inplace=True)
        
        
        debit_columns = ['CB', 'CB en ligne', 'Espèces', 'Chèques', 'Autres']
        ...
        df_CB.drop(["CB", "CB en ligne", "Espèces", "Chèques", "Autres", "Chèques cadeaux", "Total réglé"], axis=1, inplace=True)


        df_CHQ['Débit']=df_CHQ['Chèques cadeaux']
        df_CHQ['Crédit']=''
        df_CHQ['N°compte'] = df_CHQ['N°compte'].replace(to_replace='.*', value=4673, regex=True)
        df_CHQ.drop(["CB", "CB en ligne", "Espèces", "Chèques", "Autres", "Chèques cadeaux", "Total réglé"], axis=1, inplace=True)
...
def cheques_cadeaux():
    # Afficher le titre personnalisé
    st.title("2048 - BRUNA AROUMOUGOM : Nettoyage des Chèques-Cadeaux")
    ...
    if uploaded_file is not None:
        ...
        column_names = ["Date", "Remises TTC", "", "Chèques cadeaux émis", "Chèques cadeaux utilisés", "Chèques cadeaux clôturés", "Solde"]  # Remplacez les ... par le reste de vos noms de colonnes
        df.columns = column_names
        df['Pièce'] = 'CHQCAD'
        df['N°compte'] = 'CCHQCAD'
        df['Libellé'] = 'Chèque cadeau émis à reclasser'


        df_short = df[["Date", "Pièce", "N°compte", "Libellé", "Chèques cadeaux émis"]]
        df_short = df_short.drop(df_short.index[-1])

        df_credit = df_short.copy()
        df_debit = df_short.copy()

        df_credit['N°compte'] = '4673'
        df_credit['Débit'] = ''
        df_credit['Crédit'] = df_credit["Chèques cadeaux émis"]
        df_credit = df_credit.drop(columns=["Chèques cadeaux émis"])

        df_debit=df_debit.rename(columns={"Chèques cadeaux émis": "Débit"})
        df_debit['Crédit'] = ""

        # Ajouter une colonne "Index_Origine" contenant les index d'origine de chaque dataframe
        df_credit['Index_Origine'] = df_credit.index
        df_debit['Index_Origine'] = df_debit.index

        # Créer une colonne "Dataframe_Type" pour indiquer le type de chaque dataframe
        df_credit['Dataframe_Type'] = 'credit'
        df_debit['Dataframe_Type'] = 'debit'

        # Concaténer les dataframes
        df_concat = pd.concat([df_credit, df_debit])

        # Définissez l'ordre des catégories
        ordre_categories = ['debit', 'credit']
...
def fonds_caisse():
    # Afficher le titre personnalisé
    st.title("2048 - BRUNA AROUMOUGOM : Nettoyage des Fonds de Caisse")
    ...
    if uploaded_file is not None:
        ...
        if uploaded_file.type == 'text/csv':
            # Lecture du fichier CSV
            column_names = ["Date", "Montant à l'ouverture", "Dépot", "Mise en banque", "Retrait spécifique", "Espèces encaissées", "Régularisation", "Montant à la fermeture"]
            df = pd.read_csv(uploaded_file, dtype=str, skiprows=1, header=None, names=column_names)
        else:
            st.error("Format de fichier non pris en charge.")
            return
        ...
        df_short = df[["Date", "Pièce", "N°compte", "Libellé", "Dépot", "Mise en banque", "Retrait spécifique", "Espèces encaissées", "Régularisation"]]

        df_depot = df_short.copy()
        df_banque = df_short.copy()
        df_retrait = df_short.copy()
        df_especes = df_short.copy()
        df_regular = df_short.copy()

        df_depot['Libellé'] = 'Dépot'
        df_depot['N°compte'] = '471'
        df_depot['Débit'] = df_depot["Dépot"]
        df_depot['Crédit'] = ''
        df_depot = df_depot[["Date", "Pièce", "N°compte", "Libellé", "Débit", "Crédit"]]

        df_banque['Libellé'] = 'Mise en banque'
        df_banque['N°compte'] = '581'
        df_banque['Débit'] = ''
        df_banque['Crédit'] = df_banque["Mise en banque"].str.replace('-', '')
        df_banque = df_banque[["Date", "Pièce", "N°compte", "Libellé", "Débit", "Crédit"]]

        df_retrait['Libellé'] = 'Retrait spécifique'
        df_retrait['N°compte'] = '471'
        df_retrait['Débit'] = ''
        df_retrait['Crédit'] = df_retrait["Retrait spécifique"].str.replace('-', '')
        df_retrait = df_retrait[["Date", "Pièce", "N°compte", "Libellé", "Débit", "Crédit"]]

        df_especes['Libellé'] = 'Espèces encaissées'
        df_especes['Débit'] = df_especes["Espèces encaissées"]
        df_especes['Crédit'] = ''
        df_especes['N°compte'] = '581'
        df_especes = df_especes[["Date", "Pièce", "N°compte", "Libellé", "Débit", "Crédit"]]

        df_regular['Libellé'] = 'Régularisation'
        df_regular['Débit'] = ''
        df_regular['Crédit'] = df_regular["Régularisation"].str.replace('-', '')
        df_regular['N°compte'] = '471'
        df_regular = df_regular[["Date", "Pièce", "N°compte", "Libellé", "Débit", "Crédit"]]

        # Ajouter une colonne "Index_Origine" contenant les index d'origine de chaque dataframe
        df_depot['Index_Origine'] = df_depot.index
        df_banque['Index_Origine'] = df_banque.index
        df_retrait['Index_Origine'] = df_retrait.index
        df_especes['Index_Origine'] = df_especes.index
        df_regular['Index_Origine'] = df_regular.index


        # Créer une colonne "Dataframe_Type" pour indiquer le type de chaque dataframe
        df_depot['Dataframe_Type'] = 'DPT'
        df_banque['Dataframe_Type'] = 'BNQ'
        df_retrait['Dataframe_Type'] = 'RTR'
        df_especes['Dataframe_Type'] = 'ESP'
        df_regular['Dataframe_Type'] = 'RGL'


        # Concaténer les dataframes
        df_concat = pd.concat([df_depot, df_banque, df_retrait, df_especes, df_regular])

        # Définissez l'ordre des catégories
        ordre_categories = ['DPT', 'ESP', 'RTR', 'BNQ', 'RGL']
...
def main():
    ...
    pages = {
        ...
        "Chèques cadeaux" : cheques_cadeaux,
        "Fonds de caisse" : fonds_caisse
    }
